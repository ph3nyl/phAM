...
namespace phAM
{
    ...
    #region Patch_
    //public bool Patch_Commands { get; set; } = true;
    #endregion
    }
    public class Mod : IHarmonyMod
    {
        #region Members
        private static string modPath = Path.Combine(ModManager.ModPath, "phAM");
        public const string ID = "com.ACE.ACEmulator.phAM";
        public const string Name = "phAM";
        ...
        private void Start()
        {
            try
            {
                ...
                //if (Settings != null && Settings.Patch_Commands)
                //{
                //    Patch_Commands.Init();
        ...
        #region Commands
            [CommandHandler($"{Name}", AccessLevel.Player, CommandHandlerFlag.RequiresWorld, 2,
                $"bind or unbind a discord id to this account for account management through discord.",
                "<bind/unbind> <discord id>")]
            public static void Commands(Session session, params string[] parameters)
                => Discord.CommandHandler(session, parameters);
        #endregion
    }
